"""
TransE evaluation & visualization
학습이 끝난 후 저장된 모델을 로드해서 평가 + 시각화 수행.

Usage:
    python evaluate_visualize_transe.py
    python evaluate_visualize_transe.py --output ./transe_result --sample_size 1500
"""
...
def main():
    ...
    # ── 임베딩 추출 (TransE는 실수 벡터) ─────────────────
    # ComplEx와의 핵심 차이:
    #   ComplEx → 복소수(real+imag) → concat 해서 실수 벡터화 필요
    #   TransE  → 처음부터 실수 벡터 → .real / .imag 처리 불필요
    ...
    # 혹시라도 complex dtype으로 떨어지는 경우(드물지만) 안전장치
    if np.iscomplexobj(entity_embeddings):
        entity_embeddings = np.concatenate(
            [np.real(entity_embeddings), np.imag(entity_embeddings)], axis=-1
        )
    ...
    fig = px.scatter_3d(
        df_3d, x='x', y='y', z='z',
        text='entity', color='x',
        title='TransE Entity Embeddings (3D t-SNE) - Sampled',
        labels={'x': 'TSNE-1', 'y': 'TSNE-2', 'z': 'TSNE-3'},
    )
    fig.update_traces(marker=dict(size=5), textposition='top center')
    fig.update_layout(margin=dict(l=0, r=0, b=0, t=40))

    html_path = os.path.join(OUTPUT, 'transe_3d_visualization.html')
